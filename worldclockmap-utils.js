(function (root, factory) {
  ...
  function createMapSvg(date, clocks, getTime, mapUrl = "worldOutlineLow.svg", mapFit = "contain") {
    ...
    const markers = clocks.filter((clock) => Number.isFinite(clock.latitude) && Number.isFinite(clock.longitude)).map((clock) => {
      ...
      return `<g class="wcm-marker" data-side="${anchor}" transform="translate(${x} ${y})"><g class="wcm-marker-content"><ellipse rx="4.5" ry="4.5"/><text x="${dx}" y="-4" text-anchor="${anchor}">${escapeHtml(clock.label || clock.timeZone)}</text><text class="wcm-marker-time" x="${dx}" y="12" text-anchor="${anchor}">${escapeHtml(getTime(clock))}</text></g></g>`;
    }).join("");
